# Sunday Brief: the Cowork task prompt

The Sunday Brief is a Cowork task James runs by hand each week. It reads the
week's AI newsletters in the magnumai.newsletters@gmail.com inbox and writes
one briefing. This file holds the current prompt in full so it is versioned
and the next session can see what the brief is supposed to produce.

Rewritten 4 Sep 2026. The change from the first version: the six mandatory
sections became five, WHAT CHANGED and the model scoreboard went, a fifth
relevance test was added, and a third output feeds the members area.
...
2. Any BUILD THIS item that a client could use without James in the room is
   also drafted as a shelf card, generic, in the block headed FOR THE SHELF
   at the foot of the email.
3. James reads the card, fixes anything wrong, and pastes the object into the
   `S` array in `prompts/index.html`. Nothing reaches the members area
   without that step.
...
The private vault (prompts-library) keeps the full record with sources. The
shelf is its curated, generic, approved subset.
...
1. The briefing, sent live to james@magnumai.com.au using the outlook-send skill (Make webhook, subject "Sunday Brief - [date]"). This is a standing scheduled send with pre-approval; the per-send confirmation gate is waived for this recipient and this recipient only. Close the email with one line: X emails read in full, Y skipped as promos or non-AI mail, Z fetched from web due to truncation.
2. Below that closing line, a block headed FOR THE SHELF, DRAFT. It holds every item tagged FOR THE SHELF this week, zero to two cards, formatted as JavaScript objects James can paste into the shelf page without editing. If there are none, the block is the single line "Nothing for the shelf this week." Use this shape exactly:
   ...
   The levers array lists only the levers the prompt actually pulls on. The prompt text is what a client would paste, so it contains no placeholder the client cannot fill. Bracketed fill-ins like [your business] are fine.
3. Silently append raw material to the four vault files: tools-library, prompts-library, content-ideas, sales-lessons. Full detail goes here including sources. If the run has no Mac access, attach append-ready blocks to the session workspace and note it in the closing line.
